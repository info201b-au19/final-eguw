cost_of_living <- tabPanel(
  ...
  titlePanel(em("Is Homeless rate realted to Cost of Living?")),
  sidebarLayout(
    sidebarPanel(
      ...
      selectInput(
        ...
        choices = c("Cost.of.Living.Index", "Rent.Index", "Cost.of.Living.Plus.Rent.Index", "Groceries.Index", "Local.Purchasing.Power.Index", "Monthly.Pass"),
    ...
    mainPanel(
      ...
      p("This bar chart includes 10 representative cities ranging from cities
        with below average CPIs(or your choice of Index) to those with the higest
        CPIs. As you can tell, as the CPI goes up, the Homelessness rates goes up.
        We have calculated the number of Homeless people in 1 Million people to
        eliminate the influence by city population(larger cities tend to have
        higher CPIs and higher population, which means more homeless people). Note
        that Atlanta and Seattle are two outlier in this comparison, with unusual
        high population of Homeless. However, this can't deny that Homeless rate is
        correlated with CPI. We should be cocerned that Seattle's high homeless
        population has something to do with other factors such as lack of
        governance and bad drug control, etc.", style = "font-size:16pt"
      )
    )
  )
)
